...
import {origin,person,website,modified,currentRole} from './seo-config.mjs';
...
const roleBadge=`<p class="current-role">${e(currentRole.title)} at <a href="${e(currentRole.source)}" target="_blank" rel="noopener noreferrer">${e(currentRole.organization.name)} <span aria-hidden="true">↗</span></a></p>`;
...
const footer=`<footer class="directory-footer"><p>Himanshu Narwaria<br><span>${e(currentRole.title)} at <a href="${e(currentRole.source)}" target="_blank" rel="noopener noreferrer">${e(currentRole.organization.name)}</a><br>Agra, India · Working worldwide</span></p><nav aria-label="Footer navigation"><a href="services.html">All services</a><a href="about.html">About Himanshu</a><a href="index.html#blog">Design notes</a><a href="https://www.linkedin.com/in/himanshunarwaria" target="_blank" rel="noopener noreferrer">LinkedIn</a><a href="https://www.instagram.com/himanshunarwaria" target="_blank" rel="noopener noreferrer">Instagram</a></nav></footer>`;
...
for(const s of services){
  ...
  const body=`${breadcrumb(s.name,true)}
    <section class="service-hero" aria-labelledby="service-title">
      <div><p class="directory-eyebrow">${e(s.name)} · Agra, India</p>${roleBadge}<h1 id="service-title">${e(s.headline)}</h1><p class="directory-lead">${e(s.intro)}</p><a class="directory-button" href="#project-contact">Discuss your project <span aria-hidden="true">↗</span></a></div>
...
home=home.replace(/<!-- current-role:start -->[\s\S]*?<!-- current-role:end -->/,`<!-- current-role:start -->\n            <a class="hero-role" href="${e(currentRole.source)}" target="_blank" rel="noopener noreferrer"><span class="hero-role-title">${e(currentRole.title)} at <strong>${e(currentRole.organization.name)}</strong></span><span class="hero-role-proof">Meet the team <span aria-hidden="true">↗</span></span></a>\n            <!-- current-role:end -->`);
...
home=home.replace(/<script type="application\/ld\+json">[\s\S]*?<\/script>/,`<script type="application/ld+json">${json({'@context':'https://schema.org','@graph':[person,website,{'@type':'WebPage','@id':origin+'/#webpage',url:origin+'/',name:'Himanshu Narwaria | Xavik Labs Co-founder & Designer',isPartOf:{'@id':website['@id']},about:ref,mainEntity:ref},{'@type':'Service','@id':origin+'/#service',name:'Creative and digital services by Himanshu Narwaria',provider:ref,url:origin+'/services.html',hasOfferCatalog:{'@type':'OfferCatalog',name:'Creative services',itemListElement:services.map(s=>({'@type':'Offer',itemOffered:serviceEntity(s)}))}}]})}</script>`);
